package com.dubu.download.engine.util;
...
import com.dubu.download.engine.bean.AssetInfo;
import com.dubu.download.engine.constants.AndroidOptOS;
...
public class PathTool {
    ...
    private static final String TAG = PathTool.class.getName();
    ...
    //allen 新增 获取我们自己的app 的下载  目录
    private static String AI_ASSET_DOWNLOAD_DIRECTORY = SDK_FILE_ROOT_DIRECTORY + "AIAy";

    private static String ASSET_DOWNLOAD_AI_MUSIC_BG = AI_ASSET_DOWNLOAD_DIRECTORY + File.separator + "AiMusic";
    private static String ASSET_DOWNLOAD_AI_USER_VIDEO = AI_ASSET_DOWNLOAD_DIRECTORY + File.separator + "AiUserVideo";
    private static String ASSET_DOWNLOAD_AI_USER_IMAGE = AI_ASSET_DOWNLOAD_DIRECTORY + File.separator + "AiUserImage";
    ...
    public static String getFolderDirPath(String dstDirPathToCreate) {
        ...
        File dstFileDir = new File(Environment.getExternalStorageDirectory(), dstDirPathToCreate);
        if (AndroidOptOS.USE_SCOPED_STORAGE) {
            dstFileDir = new File(BaseApp.instance.getExternalFilesDir(""), dstDirPathToCreate);
        }
    ...
    public static String getInnerSDCardPath() {
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            File external = BaseApp.instance.getExternalFilesDir(null);
            if (external != null) {
                return external.getAbsolutePath() + File.separator;
            }
        }
        return BaseApp.instance.getFilesDir().getAbsolutePath() + File.separator;
    }
    ...
    public static String getAssetDownloadPath(int assetType) {
        ...
        switch (assetType) {
            ...
            //allen 新增 获取我们自己的app 的下载背景音乐的 目录
            case AssetInfo.ASSET_AI_MUSIC_BG:
                assetDownloadDir = getAssetDownloadDirPath(ASSET_DOWNLOAD_AI_MUSIC_BG);
                break;
            //allen 新增 获取我们自己的app 的下载用户视频数据(图片 配音)的 目录
            case AssetInfo.ASSET_AI_USER_VIDEO:
                assetDownloadDir = getAssetDownloadDirPath(ASSET_DOWNLOAD_AI_USER_VIDEO);
                break;
            case AssetInfo.ASSET_AI_USER_IMAGE:
                assetDownloadDir = getAssetDownloadDirPath(ASSET_DOWNLOAD_AI_USER_IMAGE);
                break;
        }
    ...
    public static String getAiAyDownloadDir() {
        return getFolderDirPath(AI_ASSET_DOWNLOAD_DIRECTORY);
    }
