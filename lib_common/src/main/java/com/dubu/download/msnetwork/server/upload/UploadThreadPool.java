 ...
 */
package com.dubu.download.msnetwork.server.upload;

import com.dubu.download.msnetwork.server.task.PriorityBlockingQueue;
 ...
 * 创建日期：2016/1/26
 * 描    述：上传管理的线程池
 * Upload the managed thread pool
 * 修订历史：
 * ================================================
 */
public class UploadThreadPool {
    private static final int MAX_IMUM_POOL_SIZE = 5;     //最大线程池的数量 Maximum number of thread pools
    ...
    private int corePoolSize = 1;                        //核心线程池的数量，同时能执行的线程数量，默认1个 The number of core thread pools and the number of threads that can execute at the same time, by default, is 1
            ...
            synchronized (UploadThreadPool.class) {
                if (executor == null) {
                    executor = new XExecutor(corePoolSize, MAX_IMUM_POOL_SIZE, KEEP_ALIVE_TIME, UNIT, //
                                             new PriorityBlockingQueue<Runnable>(),   //无限容量的缓冲队列 Buffer queues of unlimited capacity
                                             Executors.defaultThreadFactory(),        //线程创建工厂 Thread creation factory
                                             new ThreadPoolExecutor.AbortPolicy());   //继续超出上限的策略，阻止 Continue the above - limit strategy, stop
                }
            }
        }
        return executor;
