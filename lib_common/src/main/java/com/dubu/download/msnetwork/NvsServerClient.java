...
import com.dubu.download.msnetwork.callback.AbsCallback;
import com.dubu.download.msnetwork.callback.Callback;
import com.dubu.download.msnetwork.custom.BaseResponse;
import com.dubu.download.msnetwork.custom.RequestCallback;
import com.dubu.download.msnetwork.https.HttpsUtils;
import com.dubu.download.msnetwork.model.Progress;
import com.dubu.download.msnetwork.model.Response;
import com.dubu.download.msnetwork.request.GetRequest;
import com.dubu.download.msnetwork.request.PostRequest;
import com.dubu.download.msnetwork.request.base.Request;
import com.dubu.download.msnetwork.server.OkDownload;
import com.dubu.download.msnetwork.server.download.DownloadListener;
import com.dubu.download.msnetwork.server.download.DownloadTask;
import com.dubu.download.msnetwork.server.task.XExecutor;
import com.dubu.download.msnetwork.temp.TempStringCallBack;
import com.dubu.download.msnetwork.utils.OkLogger;
...
public final class NvsServerClient {
    ...
    public static final String DEFAULT_URL = MsAiClient.getMsHttpUrl();
    /**
     * 默认服务器地址
     * Default server address
     */
    private String defaultUrl = MsAiClient.getMsHttpUrl2();
     ...
     * 所有任务执行完成 回调 添加
     */
    public void onAllTaskEnd(XExecutor.OnAllTaskEndListener listener) {
     ...
     * Cancel all requests
     */
    public void removeAllTaskAllen() {
        OkDownload.getInstance().removeAllAllen(false);
    }
}
