...
import com.dubu.wheel.R

class MultiStateAiView
@JvmOverloads constructor(context: Context,
    ...
    init {
        ...
        val a = getContext().obtainStyledAttributes(attrs, R.styleable.MultiStateAiAiView)

        val loadingViewResId = a.getResourceId(R.styleable.MultiStateAiAiView_msv_loadingView, -1)
        ...
        val emptyViewResId = a.getResourceId(R.styleable.MultiStateAiAiView_msv_emptyView, -1)
        if (emptyViewResId > -1) {
        ...
        val errorViewResId = a.getResourceId(R.styleable.MultiStateAiAiView_msv_errorView, -1)
        if (errorViewResId > -1) {
        ...
        viewState = when (a.getInt(R.styleable.MultiStateAiAiView_msv_viewState, VIEW_STATE_CONTENT)) {
            VIEW_STATE_ERROR -> ViewState.ERROR
            VIEW_STATE_EMPTY -> ViewState.EMPTY
            VIEW_STATE_LOADING -> ViewState.LOADING
            else -> ViewState.CONTENT
        }
        animateLayoutChanges = a.getBoolean(R.styleable.MultiStateAiAiView_msv_animateViewChanges, false)
        a.recycle()
    }

    /**
     * Returns the [View] associated with the [com.hikennyc.view.MultiStateAiView.ViewState]
     *
     * @param state The [com.hikennyc.view.MultiStateAiView.ViewState] with to return the view for
     * @return The [View] associated with the [com.hikennyc.view.MultiStateAiView.ViewState], null if no view is present
     */
     ...
     * @param state         The [com.hikennyc.view.MultiStateAiView.ViewState]to set
     * @param switchToState If the [com.hikennyc.view.MultiStateAiView.ViewState] should be switched to
     */
    ...
    /**
     * Sets the [View] for the given [com.hikennyc.view.MultiStateAiView.ViewState]
     *
    ...
    /* All of the addView methods have been overridden so that it can obtain the content view via XML
     It is NOT recommended to add views into MultiStateAiView via the addView methods, but rather use
    ...
    /**
     * Shows the [View] based on the [com.hikennyc.view.MultiStateAiView.ViewState]
     */
